...
do.plot <- function(a.data.frame, chosen.params){
  ...
  somex <- seq(min(thex1), max(thex1), by=.1)
  ...
  for (i in 1:length(somex)){
    for (j in 1:length(somex)){
      #keep <- c(somex[i], somex2[j], somex[i]^2, somex2[j]^2, somex[i] + somex[j])
      keep <- c(1, somex[i], somex2[j])
      z[i, j] <- t(as.matrix(keep)) %*% chosen.params
    }
  }

  maybe <- data.frame(x=somex, y=somex2, z=z)

  
  somey <- -19.4945780 + (0.1644228 * somex) + (0.1535174 * somex2)
  another <- data.frame(x=somex, y=somey)
...
#     stat_contour(x=maybe$x, y=maybe$y, z=maybe$z)
   # stat_function(fun=function(x){sin(x)*4 + 40}, n=300)
    #stat_function(fun=function(x){(0.1644228*x) -19.4945780}, n=300)
    #geom_point(aes(y=thex2))

  plot(a.data.frame$exam2 ~ a.data.frame$exam1,  pch=20, 
       col=c("red","green3")[a.data.frame$admitted+1])
...
new.mat <- as.matrix(training[,c(1,2)])
new.mat <- cbind(new.mat, new.mat[,1] ^ 2)
new.mat <- cbind(new.mat, new.mat[,2] ^ 2)
new.mat <- cbind(new.mat, (new.mat[,1] + new.mat[,2]))
...
result <- optim(par=rep(0, ncol(new.mat)+1),
                cost.function, 
                get.gradient,
                x.mat=new.mat,
                y.vec=as.matrix(training[,3]), lambda=0,
